The repo we forked this from was built to demonstrate "three advanced developer testing techniques as they apply to the [Gilded Rose Kata](http://craftsmanship.sv.cmu.edu/exercises/gilded-rose-kata)."  You can learn more about those techniques in the parent repo.

We've restructured the repo to provide a quick-start introduction to refactoring techniques by replacing the refactored Gilded Rose Kata code with the original kruft (translated into Ruby by professor - check it out here: https://github.com/professor/GildedRose), but retaining the awesome characterization tests amckinnell built.
...
Stretch goals
If you want to make things a little more interesting, try ensuring the following items to the Gilded Lily's behave properly:
* "Aged Stilton", which should act like brie.
